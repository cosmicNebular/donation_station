...
struct CampaignDetails {
    ...
    pub donated_people: Vec<DonatedUser>,
}
...
fn create_campaign(program_id: &Pubkey, accounts: &[AccountInfo], instruction_data: &[u8]) -> ProgramResult {
    ...
    input_data.donated_people = Vec::<DonatedUser>::new();
...
fn donate(program_id: &Pubkey, accounts: &[AccountInfo], _instruction_data: &[u8]) -> ProgramResult {
    ...
    let mut campaign_data = CampaignDetails::try_from_slice(*writing_account.data.borrow()).expect("Error deserialaizing data");

    let donated_amount = **donator_program_account.lamports.borrow();
    if campaign_data.donated_people.len() > 100 {
        return Err(ProgramError::InvalidArgument);
    }
    campaign_data.amount_donated += donated_amount;
    let donated_user = DonatedUser{
        address: *donator.key,
        amount: donated_amount,
    };
    campaign_data.donated_people.push(donated_user);
