...
import {
    ...
    Transaction,
    PublicKey,
    TransactionInstruction, TransactionInstructionCtorFields
} from "@solana/web3.js";
...
const programId = new PublicKey(
    "FoGgzgSojwBErFKQQbsTunARmJdWYefqCqQZixJw9bzH"
);
...
export async function setPayerAndBlockhashTransaction(instructions: any) {
    ...
    transaction.feePayer = wallet.publicKey as PublicKey | undefined;
...
export async function getAllCampaigns() {
    ...
    accounts.forEach((e) => {
        try {
            let campData: any = deserialize(CampaignDetails.schema, CampaignDetails, e.account.data);
            x.push({
                pubId: e.pubkey,
                name: campData.name,
...
export async function donateToCampaign(campaignPubKey: PublicKey, amount: number) {
    await checkWallet();
...
class WithdrawRequest {
    ...
    static schema: any = new Map([[WithdrawRequest,
        {
            kind: 'struct',
            fields: [
                ['amount', 'u64'],
...
export async function withdraw(campaignPubKey: PublicKey, amount: number) {
    await checkWallet();
